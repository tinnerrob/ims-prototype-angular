...
There are **no Karma specs** — verification is `npm run check:store`, 26 harnesses
that compile the core services to JS and drive the *real* store from Node (no
browser, no server) — one of them instantiating a component to reach its view math —
